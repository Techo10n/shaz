 ...
 const sendMessage = async () => {
   ...
   try {
     ...
     if (user) {  // Only save if the user is authenticated
       const userDocRef = doc(db, 'users', user.uid); // Use user ID as document ID
      ...
      //  const userDoc = await getDoc(userDocRef);
       ...
       // let existingText = '';
       // if (userDoc.exists()) {
       //   existingText = userDoc.data()?.text || ''; // Get the existing text or an empty string
       // }

       const historyCollectionRef = collection(userDocRef, 'history');
      
       const noteDocRef = doc(historyCollectionRef)

       const noteData = {
         title: 'My First Post',
         content: userMessage,
         date_time: new Date(), // Adding a timestamp if needed
       };

       try {
         await setDoc(noteDocRef, noteData); // Auto-generate document ID
