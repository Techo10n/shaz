...
const handleLogin = async () => {
 ...
 try {
   ...
   await setDoc(userDocRef, { // make a new document within users. document id is from user.uid (specific to email that logged in).
     userId: loggedInUser.uid,       // Save user ID
     numNotes: 0
   });
   //make subcollection (history) reference
   // const postsCollectionRef = collection(userDocRef, 'history');
   //within history (subcollection), make a document for each new note. each will have fields, look at home page tsx file
